...
function addBillingAddr( $addr )
{
	...
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(10,4, "Shipping Address", 0, 0, "");
...
function addBanckDetails($bank)
{
	...
	$this->SetXY( $r1 , $y1+5 );
	$this->Cell($length,4, "IFSC CODE: ".$bank['our_ifsc_code'].", A/C No: ".$bank['our_banck_ac']);
}
	...
	if ( $params["AccompteExige"] == 1 )
	{
		if ( $params["accompte"] > 0 )
		{
			...
			if ( strlen ($params["Remarque"]) == 0 )
				$this->addBanckDetails( "Accompte de $accompteTTC Euros exig� � la commande.");
			else
		...
		else if ( $params["accompte_percent"] > 0 )
		{
			...
			if ( strlen ($params["Remarque"]) == 0 )
				$this->addBanckDetails( "Accompte de $percent100 % (soit $accompteTTC Euros) exig� � la commande." );
			else
				$this->addBanckDetails( $params["Remarque"] );
		}
		else
			$this->addBanckDetails( "Dr�le d'acompte !!! " . $params["Remarque"]);
	}
	else
	{
		if ( strlen ($params["Remarque"]) > 0 )
