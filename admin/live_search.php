...
union (select name,pic from cabinet_tbl where name like '%{$search}%')
union (select name,pic from prebuilt_tbl where name like '%{$search}%')
union (select name,pic from user_login where name like '%{$search}%') LIMIT 5";
...
if(mysqli_num_rows($result) > 0 ){
  $output = "<table border='0' style='color:black;background-color:white;' class='table'>
              ...
              while($row = mysqli_fetch_assoc($result)){
                $output .= "<tr>
                <td align='center'><img src='../search/{$row["pic"]}' width='40px'>&nbsp;{$row["name"]} </td></tr>";
              }
    $output .= "</table>";
